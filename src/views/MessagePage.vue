...
Vue.use(Contextmenu);
import {
  chatListsServ, clearChatUnreadNumServ, delChatItemServ, topChatItemServ, setNotDisturbServ
} from "@/api/chat";
import {
  removeFriendServ, friendRemarkEditServ
} from "@/api/user";
import {
  secedeGroupServ
} from "@/api/group";
import {
  packTalkItem, beautifyTime, addClass, removeClass
...
export default {
  ...
  data(){
    return {
      launchGroupShow: false, // 当前聊天的对话索引名称
      index_name: null, // 对话面板的传递参数
      params: {
        source: 0,
        receiveId: 0,
        nickname: '',
  ...
  computed: {
    // 计算置顶栏目的高度
    subHeaderPx(){
      let num = this.topItems.length, len = 65, n = 7; // 一排能显示的用户数
      if(num > n){
        let y = (num % n) > 0 ? 1 : 0;
  ...
  methods: {
    ...
    getIndex(index_name){
      return this.$store.state.talks.items.findIndex(item => item.index_name == index_name);
    }, // 切换聊天栏目
    ...
    talkItemsMenu(data, event){
      ...
      let items = {
        items: [{
          ...
          icon: item.not_disturb == 0 ? "el-icon-close-notification" : "el-icon-bell",
          onClick: () => {
